...
from django.views.generic import View
from organizer.forms import PostForm
...
class PostUpdate(View):
	...
	def get_object(self, year, month, slug):
		return get_object_or_404(self.model, pub_date__month=month, pub_date__year=year, slug=slug)
...
class PostDelete(View):
	...
	def get(self, request, year, month, slug):
		post = get_object_or_404(Post, pub_date__month=month, pub_date__year=year, slug=slug)
		return render(request, 'blog/post_confirm_delete.html', {'post': post})

	def post(self, request, year, month, slug):
		post = get_object_or_404(Post, pub_date__month=month, pub_date__year=year, slug=slug)
		post.delete()
		return render('blog_list')
